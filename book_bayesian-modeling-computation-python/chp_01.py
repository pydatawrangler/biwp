...
def _(mo):
    mo.md(r"""# Code 1: Bayesian Inference""")
    return
...
@app.cell
def _():
    # Set PyTensor C++ compiler to clang++.  This is needed for M1/M2 Macs.
    import pytensor
...
def _(az, np, plt):
    ...
    np.random.seed(521)
    viridish = [
        (0.2823529411764706, 0.11372549019607843, 0.43529411764705883, 1.0),
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""## A DIY Sampler, Do Not Try This at Home""")
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""### Code 1.4 and Figure 1.2""")
    return
...
def _(plt, trace):
    _, _axes = plt.subplots(
        1, 2, figsize=(10, 4), constrained_layout=True, sharey=True
    )
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""## Say Yes to Automating Inference, Say No to Automated Model Building""")
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""## A Few Options to Quantify Your Prior Information""")
    return
...
def _(idata, model, pm):
    pred_dists = (
        pm.sample_prior_predictive(1000, model).prior_predictive["y_obs"].values,
        pm.sample_posterior_predictive(idata, model)
...
def _(Y, az, idata, plt, pred_dists, stats):
    predictions = (
        stats.binom(n=1, p=idata.posterior["θ"].mean()).rvs((4000, len(Y))),
        pred_dists[1],
    )
    for d, _c, l in zip(
        predictions, ("C0", "C4"), ("posterior mean", "posterior predictive")
    ):
        _ax = az.plot_dist(
            d.sum(-1),
            label=l,
            figsize=(10, 5),
            hist_kwargs={"alpha": 0.5, "color": _c, "bins": range(0, 22)},
        )
        _ax.set_yticks([])
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""### Code 1.8 and Figure 1.7""")
    return
...
def _(np, plt, stats, viridish):
    _, _axes = plt.subplots(
        2, 3, figsize=(12, 6), sharey=True, sharex=True, constrained_layout=True
    )
...
def _(np, plt):
    ...
    n = 7
    _, _axes = plt.subplots(
        2, 2, figsize=(10, 5), sharex="col", sharey="row", constrained_layout=False
    )
    ...
    _axes[1, 1].text(
        ...
        0.03,
        size=18,
        s="$p(\\theta \\mid Y) \\, \\frac{d\\theta}{d\\kappa}$",
    )
    _axes[1, 1].annotate(
        "",
        xy=(-0.5, 0.025),
    ...
    _axes[1, 1].text(
        ...
        0.007,
        size=18,
        s="$p(\\kappa \\mid Y) \\, \\frac{d\\kappa}{d\\theta}$",
    )
    _axes[1, 1].annotate(
        "",
        xy=(-4.5, 0.015),
...
def _(az, entropies, max_ent, np, plt, viridish):
    _, _ax = plt.subplots(
        1, 3, figsize=(12, 4), sharex=True, sharey=True, constrained_layout=True
    )
    for _i in range(3):
        az.plot_kde(
            entropies[_i][np.nonzero(entropies[_i])],
            ax=_ax[_i],
            plot_kwargs={"color": viridish[_i], "lw": 4},
        )
